...
const SIMULATION_INTERVAL_MS = 1500; // How often metrics update (1.5s)
...
const DEFAULT_SERVICES = [
    // Starts CRITICAL, immediately triggering the agent
...
// --- METRIC SIMULATION UTILITY ---

/**
 * Simulates metric fluctuation and checks status against thresholds.
 */
const updateMetricAndStatus = (service) => {
    let newValue = service.value;
    const { warn, crit } = THRESHOLDS[service.metric];

    // High volatility: random walk with larger steps (10% to 15% of current value, min step 2)
    ...
    // Only update status if not currently in remediation
    const finalStatus = service.status === 'REMEDIATING' ? 'REMEDIATING' : newStatus;

    // Update history, keeping only the last MAX_HISTORY points
    const newHistory = [
...
const App = () => {
    ...
    const getInitialState = () => {
        ...
        return { services: DEFAULT_SERVICES, logs: [] };
    };
    ...
    // --- AGENT/REMEDIATION FUNCTION (Updates local state) ---
    const startRemediation = useCallback((id, currentService, isManual = false) => {
        ...
        // Step 1: Set status to REMEDIATING and log the trigger/override
        setDashboardData(prevData => {
            if (!prevData) return null;

            const remediatingServices = prevData.services.map(s => s.id === id ? { ...s, status: 'REMEDIATING' } : s);
        ...
        // Step 2: After delay, set status back to HEALTHY and log success
        setTimeout(() => {
            setDashboardData(prevData => {
                if (!prevData) return null;
                ...
                const postRemediationServices = prevData.services.map(s => {
                    if (s.id === id) {
                        let healthyValue;
                        // Stabilize value back to a healthy baseline
                ...
                // Remove the trigger log and prepend the success log
                const logIndexToReplace = prevData.logs.findIndex(log => log.serviceName === successService.name && log.type === logType);
    ...
    // --- METRIC SIMULATION (Updates local state with Auto-Remediation) ---
    useEffect(() => {
        if (!services) return;

        // Initial setup check for immediate critical/warning states on load
        services.forEach(service => {
            if (service.status === 'CRITICAL' && service.alert_id && service.alert_id.startsWith('ALERT') && service.status !== 'REMEDIATING') {
                // Ensure initial critical service is processed immediately
        ...
        const interval = setInterval(() => {
            setDashboardData(prevData => {
                ...
                const newServices = prevData.services.map(service => {
                    ...
                    // AUTO REMEDIATION LOGIC (Agent Trigger for CRITICAL)
                    if (updatedService.status === 'CRITICAL' && service.status !== 'CRITICAL') {
                        ...
                        // Schedule the full remediation process
                        startRemediation(serviceWithAlert.id, serviceWithAlert, false);

                        // Set the service status to REMEDIATING locally for immediate visual feedback
                        return { ...serviceWithAlert, status: 'REMEDIATING' };
                    }
                    
                    // LOG WARNING STATE TRANSITION
                    ...
                    // Clear Alert ID if status moves from WARNING back to HEALTHY
